...
Это ограничение закреплено политикой безопасности страницы: после загрузки интерфейса приложению запрещены сетевые соединения и внешние ресурсы. В исходном коде также нет API отправки данных; это отдельно проверяется автотестом.
...
| Устройство и способ открытия | Результат |
|---|---|
| iPhone, iPad, Android | Создаются новые локальные копии, затем открывается системное меню сохранения |
| ПК или Mac через публичный сайт / локальную сеть | Создаются новые локальные копии и сохраняются через браузер |
| Chrome или Edge через `localhost` с разрешением на запись | Можно по явному подтверждению изменить оригиналы в выбранной папке |

Во всех режимах содержимое фотографий остаётся на устройстве. Сервер раздаёт только HTML, CSS и JavaScript интерфейса.
...
| `src/standalone/` | читаемые исходники: разметка, стили и логика по модулям |
...
- undo хранит до 3 действий в памяти текущей вкладки;
...
`verify` проверяет синтаксис всех встроенных скриптов трёх entrypoint-файлов и наличие критичных сценариев: EXIF, видео, undo, перенос между каталогами и полный запрет отправки файлов по сети.
